...
from src.baseline.episode_baseline import (
    update_episode_baseline,
    compute_episode_advantages,
    init_episode_baseline,
)
...
def test_episode_baseline_updates():
    ...
    print(
        f"Initial baseline: mean={baseline.mean_episode_return}, n={baseline.n_episodes}"
    )
    ...
    print(
        f"Match: {jnp.allclose(baseline.mean_episode_return, jnp.mean(episode_totals_1))}"
    )
    ...
    print(
        f"Match: {jnp.allclose(baseline.mean_episode_return, jnp.mean(all_episodes))}"
    )
