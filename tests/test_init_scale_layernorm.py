...
from tests.constants import (
    DEFAULT_SEED, DEFAULT_BATCH_SIZE, DEFAULT_OBS_DIM, DEFAULT_ACTION_DIM,
    INIT_SCALE_TEST_VALUES, SPARSITY_TEST_VALUES
)


class ScaledGaussianPolicy(nnx.Module):
    def __init__(self, obs_dim: int, action_dim: int, hidden_dim: int = 64, 
                 use_layernorm: bool = True, init_scale: float = 1.0, 
                 sparsity: float = 0.5, rngs: nnx.Rngs | None = None):
        if rngs is None:
            rngs = nnx.Rngs(0)
        ...
        self.w1 = nnx.Param(init_scale * sparse_init(key1, (obs_dim, hidden_dim), sparsity=sparsity))
        self.b1 = nnx.Param(jnp.zeros(hidden_dim))
        self.w2 = nnx.Param(init_scale * sparse_init(key2, (hidden_dim, action_dim), sparsity=sparsity))
...
def test_init_scales():
    ...
    print("Scale | No LN Loss | With LN Loss | Ratio | Hidden std (no LN) | Hidden std (LN)")
    print("-" * 80)
    
    for scale in scales:
        # Without LayerNorm
        policy_no_ln = ScaledGaussianPolicy(
            obs_dim=DEFAULT_OBS_DIM, action_dim=DEFAULT_ACTION_DIM, hidden_dim=64, 
            use_layernorm=False, init_scale=scale,
            rngs=nnx.Rngs(key)
        )
        ...
        policy_ln = ScaledGaussianPolicy(
            obs_dim=DEFAULT_OBS_DIM, action_dim=DEFAULT_ACTION_DIM, hidden_dim=64, 
            use_layernorm=True, init_scale=scale,
            rngs=nnx.Rngs(key)
        )
        ...
        print(f"{scale:5.1f} | {loss_no_ln:10.4f} | {loss_ln:12.4f} | {ratio:5.2f}x | "
    ...
    for sparsity in sparsities:
        ...
        policy_no_ln = ScaledGaussianPolicy(
            obs_dim=DEFAULT_OBS_DIM, action_dim=DEFAULT_ACTION_DIM, hidden_dim=64, 
            use_layernorm=False, sparsity=sparsity,
            rngs=nnx.Rngs(key)
        )
        ...
        policy_ln = ScaledGaussianPolicy(
            obs_dim=DEFAULT_OBS_DIM, action_dim=DEFAULT_ACTION_DIM, hidden_dim=64, 
            use_layernorm=True, sparsity=sparsity,
            rngs=nnx.Rngs(key)
        )
