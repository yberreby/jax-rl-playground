...
def train_with_hyperparams(lr: float, sparsity: float, seed: int) -> float:
    ...
    # Create policy
    policy = GaussianPolicy(obs_dim=4, action_dim=2, hidden_dim=FIXED_HIDDEN, rngs=nnx.Rngs(key))
...
def test_simple_vmap_hypersearch():
    ...
    print(f"Best params: lr={study.best_params['lr']:.6f}, sparsity={study.best_params['sparsity']:.3f}")
    ...
    sparsity_analysis = df.groupby("sparsity_bin")["loss"].agg(["mean", "std", "min", "count"])
    print("\nSparsity bins:")
