...
def compute_episode_metrics(states, rewards, actions, angle_threshold=0.1):
    ...
    angle_stability = jnp.std(angles[upright_mask]) if jnp.any(upright_mask) else jnp.inf
...
def plot_training_curves(output_dir, metrics_history):
    ...
    if len(metrics_history['returns']) >= 10:
        ...
        ax.plot(range(9, len(metrics_history['returns'])), moving_avg, 'b-', linewidth=2)
    ...
    if len(metrics_history['upright_fractions']) >= window:
        for i in range(window, len(metrics_history['upright_fractions'])):
            rate = np.mean([f > 0.8 for f in metrics_history['upright_fractions'][i-window:i]])
            success_rate.append(rate)
        ax.plot(range(window, len(metrics_history['upright_fractions'])), success_rate, 'g-')
    ...
    # 6. Swing-up times
    ax = axes[5]
    valid_swingups = [(i, t) for i, t in enumerate(metrics_history['swing_up_times']) if t is not None]
    if valid_swingups:
    ...
    ax.fill_between(iterations,
                     np.array(metrics_history['mean_energies']) - np.array(metrics_history['energy_stds']),
                     np.array(metrics_history['mean_energies']) + np.array(metrics_history['energy_stds']),
                     alpha=0.3)
    ...
    # 8. Action metrics
    ax = axes[7]
    ax.plot(iterations, metrics_history['action_magnitudes'], 'orange', label='Magnitude', alpha=0.7)
    ax.plot(iterations, metrics_history['action_smoothness'], 'brown', label='Smoothness', alpha=0.7)
...
def train_pendulum_with_metrics(
    ...
    csv_columns = ['iteration', 'return', 'grad_norm', 'policy_std', 'upright_fraction',
                   'swing_up_time', 'angle_stability', 'mean_energy', 'action_magnitude',
                   'action_smoothness', 'learning_rate']
    ...
    for i in range(n_iterations):
        ...
        metrics_history['action_smoothness'].append(episode_metrics['action_smoothness'])
        ...
        grad_norms_batch = [jnp.linalg.norm(g.value if hasattr(g, 'value') else g)
        ...
        if verbose and i % 10 == 0:
            print(f"Iter {i:3d} | Return: {current_return:6.3f} | "
    ...
    success_rate = sum(1 for m in eval_metrics if m['upright_fraction'] > 0.8) / len(eval_metrics)
    mean_upright = np.mean([m['upright_fraction'] for m in eval_metrics])
    mean_stability = np.mean([m['angle_stability'] for m in eval_metrics if m['angle_stability'] < np.inf])
    ...
    if best_episode_data:
        viz = PendulumVisualizer(dark_mode=True)
        # Convert to lists
        states_list = [best_episode_data['states'][j] for j in range(len(best_episode_data['states']))]
        actions_list = [best_episode_data['actions'][j] for j in range(len(best_episode_data['actions']))]
        rewards_list = [best_episode_data['rewards'][j] for j in range(len(best_episode_data['rewards']))]
        viz.create_animation(
            states=states_list,
            actions=actions_list,
            rewards=rewards_list,
...
def test_single_run():
    ...
    assert results['final_performance'] > -0.5, f"Performance too low: {results['final_performance']}"
    assert results['success_rate'] > 0.05, f"Success rate too low: {results['success_rate']}"
