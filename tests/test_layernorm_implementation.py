...
def test_layernorm_behavior():
    ...
    print(
        "Input scale | Input mean | Input std | Output mean | Output std | Amplification"
    )
    print("-" * 80)
    ...
    for scale in scales:
        ...
        ln = nnx.LayerNorm(
            num_features=hidden_dim, use_bias=False, use_scale=False, rngs=nnx.Rngs(key)
        )
        ...
        print(
            f"{scale:11.2f} | {input_mean:10.4f} | {input_std:9.4f} | "
    ...
    for sparsity in [0.0, 0.5, 0.8, 0.9, 0.95]:
        ...
        print(
            f"{sparsity:8.2f} | {active_fraction:15.4f} | {input_std:9.4f} | "
